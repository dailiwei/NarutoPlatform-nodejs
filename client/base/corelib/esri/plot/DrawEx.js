/**
 * Created by dailiwei on 3/1/2015.
 */
define("dojo/_base/declare dojo/_base/lang dojo/_base/array dojo/_base/connect dojo/_base/Color dojo/_base/window dojo/has dojo/keys dojo/dom-construct dojo/dom-style esri/kernel esri/sniff esri/toolbars/_toolbar esri/symbols/SimpleMarkerSymbol esri/symbols/SimpleLineSymbol esri/symbols/SimpleFillSymbol esri/graphic esri/geometry/jsonUtils esri/geometry/webMercatorUtils esri/geometry/Point esri/geometry/Polyline esri/geometry/Polygon esri/geometry/Multipoint esri/geometry/Rect ./drawer/PlotDrawer dojo/i18n!esri/nls/jsapi".split(" "), function (_declare, _lang, Array, _connect, color, _window, _has, _keys, _domconstruct, _domstyle, esriKernel, esriSniff, Toolbar, SimpleMarker, SimpleLine, SimpleFill, graphic, jsonUtility, _webMercatorUtils, _Point, _Polyline, _Polygon, _Multipoint, _Rect, _PlotDrawer,_dojoEsrijsapi) {
    var DrawEx = _declare(Toolbar, {
        ...
        activate: function (geometryType, options) {
            ...
            switch (geometryType) {
                ...
                //扩展的
                case Draw.RECT_FLAG:
                case Draw.TRIANGLE_FLAG:
                case Draw.ASSEMBLY_AREA:
                ...
                //扩展的
                case Draw.DOUBLE_ARROW:
                case Draw.MULTI_ARROW:

                case Draw.SIMPLE_ARROW:
                case Draw.TAILED_ARROW:
                case Draw.STRAIGHT_ARROW:
                case Draw.CUSTOM_ARROW:
                case Draw.CUSTOM_TAILED_ARROW:

                case Draw.HALF_CIRCLE:
        ...
        deactivate: function () {
            ...
            switch (this._geometryType) {
                ...
                //扩展的
                case DrawEx.RECT_FLAG:
                case DrawEx.TRIANGLE_FLAG:
                case DrawEx.ASSEMBLY_AREA:
                ...
                //扩展的
                case DrawEx.DOUBLE_ARROW:
                case DrawEx.MULTI_ARROW:
                case DrawEx.SIMPLE_ARROW:
                case DrawEx.TAILED_ARROW:
                case DrawEx.STRAIGHT_ARROW:
                case DrawEx.CUSTOM_ARROW:
                case DrawEx.CUSTOM_TAILED_ARROW:

                case DrawEx.HALF_CIRCLE:
        ...
        _normalizeRect: function (point1, point2, spatialReference) {
            ...
            return {
                x: Math.min(pt1X, pt2X),
                y: Math.max(pt1Y, pt2Y),
                width: width,
                height: height,
            ...
            switch (this._geometryType) {
                ...
                case DrawEx.FREEHAND_ARROW:
                    if (this._points.length <= 1) {
                        ...
                        //tail two points
                        var pt1 = {
                            x: this._tailFactor * len * Math.cos(k) + this._points[0].x,
                            y: this._tailFactor * len * Math.sin(k) + this._points[0].y
                        };
                        var pt2 = {
                            x: -1 * this._tailFactor * len * Math.cos(k) + this._points[0].x,
                            y: -1 * this._tailFactor * len * Math.sin(k) + this._points[0].y
                        };
                        var partialLen = (1 - this._headPercentage) * len;
                        var p1 = {
                            x: this._tailFactor * partialLen * Math.cos(k) + this._points[0].x,
                            y: this._tailFactor * partialLen * Math.sin(k) + this._points[0].y
                        };
                        var p2 = {
                            x: -1 * this._tailFactor * partialLen * Math.cos(k) + this._points[0].x,
                            y: -1 * this._tailFactor * partialLen * Math.sin(k) + this._points[0].y
                        };
                ...
                case DrawEx.POLYLINEEX:
                    if (this._points.length <= 1) {
                        ...
                        for (i = 0, len = tempArray.length - 1; i < len; i++) {
                            ...
                            pt1 = {
                                x: (this._tailFactor + i / 18 / len) * partialLen * Math.cos(angleArray[i]) + tempArray[i].x,
                                y: (this._tailFactor + i / 18 / len) * partialLen * Math.sin(angleArray[i]) + tempArray[i].y
                            };
                            pt2 = {
                                x: -1 * (this._tailFactor + i / 18 / len) * partialLen * Math.cos(angleArray[i]) + tempArray[i].x,
                                y: -1 * (this._tailFactor + i / 18 / len) * partialLen * Math.sin(angleArray[i]) + tempArray[i].y
                            };
                ...
                case DrawEx.CUSTOM_TAILED_ARROW:
                    if (this._curvePt1 && this._curvePt2) {
                        var points = this._points.concat(candidatePoint);
                        var list = _PlotDrawer.createDrawPoints(this._geometryType,points);

                        var geometry = new _Polygon(this.map.spatialReference);
                        geometry.addRing(list);
                        tempGraphic.geometry = geometry;
                        tempGraphic.setGeometry(geometry);
                        //tempGraphic.setSymbol(this.fillSymbol);

                        this._setTooltipMessage(points.length);
                ...
                case DrawEx.MULTI_ARROW:
                    ...
                    if (this._curvePt1 ) {
                        ...
                        if(points.length%3==0||points.length%3==2){
                            var list = _PlotDrawer.createDrawPoints(this._geometryType,points);
                            ...
                            var list = _PlotDrawer.createDrawPoints(this._geometryType,this._points);
        ...
        _controlPointsUpdates: function (type, graphic, controlPoints) {
            switch (type) {
                ...
                case DrawEx.FREEHAND_ARROW:
                    ...
                    var headPath = this.CreateArrowHeadPathEx(leftArray[leftArray.length - 1], {
                        x: tempArray[tempArray.length - 1].x,
                        y: tempArray[tempArray.length - 1].y
                    }, rightArray[rightArray.length - 1], this._ptCollectionLen(tempArray, 0), this._headPercentage, 15);
                    ring = [];
                    ring = ring.concat(leftArray);
                    ring = ring.concat(headPath);
                    ring = ring.concat(rightArray.reverse());
                ...
                case DrawEx.STRAIGHT_ARROW:
                    ...
                    var list = _PlotDrawer.createDrawPoints(type,controlPoints);

                    var ploygon = new _Polygon(this.map.spatialReference);
                    ploygon.addRing(list);

                        graphic.geometry = ploygon;
                        graphic.setGeometry(graphic.geometry);
                        //tempGraphic.setSymbol(this.fillSymbol);
        ...
        _onMouseDragHandler: function (inputPt) {
            if (_has("esri-touch") && !this._points.length) inputPt.preventDefault();
                ...
                switch (this._geometryType) {
                    case Draw.CIRCLE:
                        this._hideTooltip();
                        tempGraphic.geometry = _Polygon.createCircle({
        ...
        _onMouseUpHandler: function (b) {
            if (this._dragged) {
                ...
                switch (this._geometryType) {
                    ...
                    //扩展的
                    case g.RECT_FLAG:
                    case g.TRIANGLE_FLAG:
                    case g.ASSEMBLY_AREA:
        ...
        _onClickHandler: function (clickPoint) {
            ...
            switch (this._geometryType) {
                case Draw.POINT:
                case Draw.POINT_TEXT:
                    var point = clickPoint.offset(0, 0);
                    point.drawExtendType=this._geometryType;
                    this._drawEnd(point);
                    this._setTooltipMessage(0);
                    break;
                ...
                case Draw.CIRCLE:
                    ...
                    break;
                case Draw.ELLIPSE:
                    Draw = new _Polygon(map.spatialReference), this._graphic = map.graphics.add(new graphic(Draw, this.fillSymbol), !0), this._graphic.geometry = _Polygon.createEllipse({
                        center: screenPt,
                        longAxis: 48,
                        shortAxis: 24,
                        numberOfPoints: 60,
                        map: map
                    }), this._graphic.setGeometry(this._graphic.geometry), this._drawEnd(this._graphic.geometry);
                    break;
                case Draw.CURVE:
                    if (this._points.length === 1) {
                        this._tGraphic = map.graphics.add(new graphic(new _Polyline({
                ...
                case Draw.DOUBLE_ARROW:
                    if (this._points.length === 1) {
                        ...
                        geometry.drawExtendType = this._geometryType;
                        ...
                        //
                        //var graphicTempAdd = this.map.graphics.add(new graphic(geometry, this.fillSymbol), !0);
                        //this._setTooltipMessage(0);
                        //var d;
            ...
            if (mapTooltip) {
                ...
                switch (this._geometryType) {
                    ...
                    case DrawEx.DOUBLE_ARROW:
                        ...
                        if (numberOfPts == 2||numberOfPts == 3)
                        {
                            text = "单击继续添加锚点，双击结束绘制";
                        }
                        if (numberOfPts == 4)
                    ...
                    case DrawEx.MULTI_ARROW:
                        ...
                        if (numberOfPts%3 ==2||numberOfPts%3 == 0)
                        {
                            text = "单击继续添加锚点，双击结束绘制";
                        }
                        if (numberOfPts%3 == 1)
        ...
        CreateArrowHeadPath: function (pt1, headPt, pt2, controlPt, totalLength, headPercentage, headAngle) {
            ...
            if (actualLength < desireHeadLength) {
                ...
                //path.push(pt1);
                path.push({
                    x: headPt.x + headSideLength * Math.cos(verticalAngle - headAngle / 180 * Math.PI),
                    y: headPt.y + headSideLength * Math.sin(verticalAngle - headAngle / 180 * Math.PI)
                });
                path.push(headPt);
                path.push({
                    x: headPt.x + headSideLength * Math.cos(verticalAngle + headAngle / 180 * Math.PI),
                ...
                path.push({
                    x: headPt.x + leftResult * Math.cos(verticalAngle - leftAcute),
                    y: headPt.y + leftResult * Math.sin(verticalAngle - leftAcute)
                });
                path.push({
                    x: headPt.x + headSideLength * Math.cos(verticalAngle - headAngle / 180 * Math.PI),
                    y: headPt.y + headSideLength * Math.sin(verticalAngle - headAngle / 180 * Math.PI)
                ...
                path.push({
                    x: headPt.x + rightResult * Math.cos(verticalAngle + rightAcute),
                    y: headPt.y + rightResult * Math.sin(verticalAngle + rightAcute)
                });
            }
            return path;
