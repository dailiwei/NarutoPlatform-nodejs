...
define([
    ...
    function (declare, lang, html, on, domConstruct, mouse, query, _WidgetBase) {
      ...
      return declare([_WidgetBase], {
        ...
        declaredClass: 'rdijit.layout.CompsGroupTileLayoutContainer',
         ...
         *   items: [ResizableNode]
         **/
        compsList:{"Base":"基础组件","IOT":"IOT组件","Rain":"雨情组件","Water":"水情组件","Drought":"抗旱组件","Drainage":"排水组件"},//从后台返回先写死
        ...
        createGroupList:function(){
          ...
          for(var i=0;i<this.items.length;i++){
            ...
            if(!isHas){
              this.groupList.push({groupName:item.groupName,groupList:[item]});
            }
          }
        },
        ...
        currentTop:5,
        createGroupDiv:function(group, i,itemSize, cpr){
          //观察是第几组
          //创建分组title
          //计算下统计信息
          var tt ;
          ...
          var state_stop_Num = 0;
           ...
           group.groupList.forEach(lang.hitch(this, function(item, i){
             ...
             if(item.data.app_state=="1"){
               state_run_Num++;
             }else if(item.data.app_state=="0"){
                   state_stop_Num++;
             }else if(item.data.app_state=="-1"){
          ...
          if(this.ddd==1){
            var label = html.create('div', {
              'innerHTML':("<div id='comp_IOT'>"+(group.groupName?this.compsList[group.groupName]:"其他") +"</div>"),
              style:'color:#a0a0a0;font-size:14px;position: absolute;left:20px;top:'+this.currentTop+'px'
            }, this.domNode);
            this.labelList.push(label);
            this.currentTop =    tt + this.itemSize.height+this.margin;
          }else{
            var label = html.create('div', {
              'innerHTML':(group.groupName?this.compsList[group.groupName]:"其他"),
              style:'color:#a0a0a0;font-size:14px;position: absolute;left:20px;top:'+this.currentTop+'px'
            }, this.domNode);
            this.labelList.push(label);
            this.currentTop =    tt + this.itemSize.height+this.margin;
