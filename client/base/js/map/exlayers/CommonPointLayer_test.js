    ...
    function (lang,
        ...
        return declare("base.map.exlayers.CommonPointLayer", GraphicsLayer, { // 站图层
            ...
            mouseOverHandler: function (evt) {
                ...
                var node = evt.graphic.getNode();
                var g = d3.select(node).transition().ease("linear")
                .attrTween("y", function () {
                    return d3.interpolate(this.y.baseVal.value, this.y.baseVal.value-30);
                }).each("end", lang.hitch(this,function(){
                        var g = d3.select(node).transition().ease("elastic")
                            .attrTween("y", function () {
                                return d3.interpolate(this.y.baseVal.value, this.y.baseVal.value+30);
                            });
                    }));
