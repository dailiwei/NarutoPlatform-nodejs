define([
    ...
    "dojo/text!./template/FloatOneLayout.html",
    "dojo/text!./css/FloatOneLayout.css",
    "base/_BaseWidget",
    ...
    return declare("base.layout.FloatOneLayout", [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        'baseClass': "base-layout-floatonelayout",
        ...
        constructor: function (args) {
            ...
            this.floatPanelList = [];
            this.panelWidgets = [];//gen数据
            this.panelListWidgets = [];//所有的panel
            this.taskWidgets = [];
            this._bottomWidget = [];
        ...
        postCreate: function () {
            ...
            this.own(on(window.document, "click", lang.hitch(this, this.closePop)));
        },
        startup: function () {
            this.inherited(arguments);
        ...
        createMainView: function () {
            ...
            this._library.loadModule1(this.mainWidget.module).then(dojo.hitch(this, function (Module) {
                ...
                try {
                    ...
                    Logger.error("错误创建main widget:【" + cfg.id + "】," + error) ;
                }

                Logger.timeEnd("createMainWidget");
            }));
        },
        ...
        createPanelIcons: function () {
            for (var i = 0; i < this.panelWidgets.length; i++) {
                ...
                var itemDiv = domConstruct.create("div", {
                    id: (this.panelGroup[item.id].length==1)?this.panelGroup[item.id][0].id:item.id
                });
                ...
                //
                var popDiv;
                if (this.panelGroup[item.id].length > 1) {//多个的话显示 上选的按钮
                    popDiv = html.create('div', {
                        id: "pop_" + item.id,
                        'class': 'content-title333',
                        innerHTML: '<i style="cursor:pointer" title="展示"  class="fa fa-caret-up"></i>',
                        style: "height:15px"
                    }, itemDiv);
                } else {
                    popDiv = html.create('div', {
                ...
                if (this.panelGroup[item.id].length > 1) {
                    this.own(on(itemDiv, 'mouseover', lang.hitch(this, function (e) {
                        var id = e.currentTarget.id;
                        this.showPop(e, id);
                    })));
                }
                this.own(on(itemDiv, 'click', lang.hitch(this, function (evt) {
                    var widgetid = evt.currentTarget.id;
                    this.selectPanelById(widgetid);
                })));
            ...
            this.own($(this.popContent).delegate("div", "click", lang.hitch(this, this.setCurrentPanel)));

            if (commonUtils.isMobile()) {
                //默认不打开
                setTimeout(lang.hitch(this, function () {
                    if (this.panelListWidget && this.panelListWidget.length > 0) {
                        //默认第一个打开
                        this.setSelectWidget(this.panelListWidget[0]);
                        topic.publish("base/layout/tabPanel/openSelectTab", this.panelListWidget[0]);
                    }
                    setTimeout(lang.hitch(this, function () {
        ...
        createPanelWidget: function (panelWidgets) {
            if (panelWidgets.length > 0) {
                //查创建container
                this.tabPanelWidget = new TabPanel();
                this.tabPanelWidget.startup();

                domConstruct.place(this.tabPanelWidget.domNode, this.panelContainer);

                //解析下数据
                var list = panelWidgets;
                this.panelListWidget = [];
                for (var i = 0, max = list.length; i < max; i++) {
                    var containerId = list[i].id;
                    var widgets = window.currentPageWidgets;
                    ...
                    for (var j = 0; j < widgets.length; j++) {
                        ...
                        if (item.container == containerId && item.parameters.region == "view") {
                            group.push(item);
                            this.panelListWidget.push(item);
                        }
                    }
                    this.panelGroup[containerId] = group;
                }
            }
        },
...
//			html.setStyle(this.bottomDiv,"display","block");
        ...
        showPop: function (e, id) {

            if (this.panelGroup.hasOwnProperty(id)) {

                var list = this.panelGroup[id];
                var str = "";
                for (var i = 0, max = list.length; i < max; i++) {
                    var item = list[i];
                    str += "<div id='" + item.id + "' class='fa-hover'><a style='padding-left: 10px;'  ><i class='fa fa-th-large' style=' '></i><span style='padding-left: 4px;'  >" + item.parameters.i18nLabel + "</span></a></div>";
                }


                this.popContent.innerHTML = str;

                var left = e.currentTarget.offsetLeft;
                $(".popover").attr("data-opt", "xxx");
                //$(".popover").css("bottom", 73).css("left",left - e.currentTarget.scrollWidth / 2 - 32 ).show();
                $(".popover").css("bottom", 73).show();

                if(this.isFirstPop){
                    this.doDnimate(this.popoverContainer, {  left: {end: left - e.currentTarget.scrollWidth / 2 - 19}  },200);
                }else{
                    $(".popover").css("bottom", 73).css("left",left - e.currentTarget.scrollWidth / 2 - 19 ).show();
                    this.isFirstPop = true;
                }
            }

        },
        setCurrentPanel: function (e) {
            this.closePop();

            var id = (e.currentTarget.id);
            this.selectPanelById(id);
        },
        selectPanelById: function (widgetid) {
            var widgetid = widgetid;
            for (var i = 0; i < this.panelListWidget.length; i++) {
                if (widgetid == this.panelListWidget[i].id) {
                    this.setSelectWidget(this.panelListWidget[i]);
                    topic.publish("base/layout/tabPanel/openSelectTab", this.panelListWidget[i]);
                    break;
                }
            }
        }
