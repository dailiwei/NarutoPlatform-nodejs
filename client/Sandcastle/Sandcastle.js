...
package_path = package_path.substring(0, package_path.lastIndexOf('/'));
require({
    packages: [
        {
            name: "dojo",
            location: package_path+"/base/corelib/dojo"
        },
        {
            name: "dijit",
            location:  package_path+"/base/corelib/dijit"
        },
        {
            name: "dojox",
            location:  package_path+"/base/corelib/dojox"
        }, {
            name: 'Sandcastle',
            location:  package_path+'/Sandcastle'
        }, {
            name: 'CodeMirror',
            location:  package_path+'/Sandcastle/vendor/codemirror-4.6'
        },
        {
            name: "base",
            location:  package_path+"/base/js"
        }
    ...
    var packages = [
        {
            name: "dojo",
            location: "/rich/base/corelib/dojo"
        },
        {
            name: "dijit",
            location: "/rich/base/corelib/dijit"
        },
        {
            name: "dojox",
            location: "/rich/base/corelib/dojox"
        }, {
            name: 'Sandcastle',
            location: '/rich/Sandcastle'
        }, {
            name: 'CodeMirror',
            location: '/rich/Sandcastle/vendor/codemirror-4.6'
        },
        {
            name: "base",
            location: "/rich/base/js"
        },
        {
            name: "simple",
            location: "/rich/simple"
        }
    ...
    var showContainer = registry.byId('showContainer');
    ...
    galleryErrorMsg.textContent = '没有找到相关Demo.';
    ...
    function openDocTab(title, link) {
        if (!defined(docTabs[title])) {
            ...
            showContainer.selectChild(docTabs[title]);
        } else {
    ...
    CodeMirror.commands.runCesium = function (cm) {
        clearErrorsAddHints();
        clearRun();
        showContainer.selectChild(bucketPane);
    ...
    cssEditor = CodeMirror.fromTextArea(document.getElementById('cssBody'), {
        mode: 'text/css',
        lineNumbers: true,
        matchBrackets: true,
        indentUnit: 4,
    ...
    registry.byId('codeContainer').watch('selectedChildWidget', function (name, oldPane, newPane) {
        if (newPane.id === 'jsContainer') {
            ...
            htmlEditor.focus();
        } else if (newPane.id === 'cssContainer') {
            cssEditor.focus();
        }
    });
    ...
    function loadFromGallery(demo) {
        ...
        return requestDemo(demo).then(function (list) {
            demo.list = list;
            ...
            var scriptCode = demo.list[1];
            demoJs = scriptCode.replace(/\s/g, '');
            jsEditor.setValue(scriptCode);
            jsEditor.clearHistory();

            //var doc = parser.parseFromString( demo.list[0], 'text/html');
            var htmlText = '';
            //var childIndex = 0;
    ...
    window.addEventListener('popstate', function (e) {
        if (e.state && e.state.name && e.state.code) {
            ...
            document.title = e.state.name + ' - RichwayPlatform';
        }
    }, false);
    ...
    registry.byId('buttonNew').on('click', function () {
        var htmlText = (htmlEditor.getValue()).replace(/\s/g, '');
        var cssText = (cssEditor.getValue()).replace(/\s/g, '');
        var jsText = (jsEditor.getValue()).replace(/\s/g, '');
        var confirmChange = true;
        if (demoHtml !== htmlText || demoJs !== jsText|| demoCss !== cssText) {
        ...
        if (confirmChange) {
            loadFromGallery(newDemo).then(function () {
                ...
                document.title = newDemo.name + ' - RichwayPlatform';
            });
        }
    });
    ...
    function requestDemo(item) {
        //simple/demo_/demoView/xxxx
        var path = item.path;
        var ss = path.split("/");
        var ps = ss[0];
        ...
        path = ps+"/" +tempPath.substring(0,tempPath.length-1);

        var name = ss[ss.length-1];
        var pss = path.replace(name,'');
        var htmlPath = ""+ pss+"template/"+name+".html";
        var jsPath = ""+ pss+name+".js";
        var cssPath = ""+ pss+"css/"+name+".css";
        var ary = [];
        return getRequestJSCSSHTML(htmlPath,jsPath,cssPath);
    }

    function getRequestJSCSSHTML(htmlPath,jsPath,cssPath){
        ...
        all([html(),js(),css()]).then(function (results) {
    ...
    function loadDemoFromFile(index) {
        ...
        return requestDemo(demo).then(function (list) {
            ...
            demo.bucket =  'bucket-requirejs.html';
            demo.description =   '';
            demo.label =  '';
            ...
            if (defined(queryObject.src)) {
                if (demo.name === queryObject.src.replace('.html', '')) {
                    loadFromGallery(demo).then(function () {
                        ...
                        document.title = demo.name + ' - RichwayPlatform';
                    });
                }
            }
